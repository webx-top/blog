...
(function($, undefined) {
	...
	$.testRemind = (function() {
		...
		return {
			id: "validateRemind",
			display: false,
			css: {padding:"8px 10px",borderColor:"#ccc",color:"#666",borderRadius:5,boxShadow: "2px 2px 4px rgba(0,0,0,.2)",backgroundColor:"#fff",lineHeight:"24px",fontSize:13},
			hide: function() {
				$("#" + this.id).remove();
				this.display = false;
				$(document).unbind({
	...
	OBJREG = {
		...
		"prompt": {
			...
			number: Tr("请输入数值"),
			date: Tr("请输入日期"),
			pattern: Tr("内容格式不符合要求"),
			empty: Tr("请填写此字段"),
			multiple: Tr("多条数据使用逗号分隔")
		}
	};
				  ...
				  , remind = function(control, type, tag) {
					...
					if ($(control).isVisible()) {
						if (type == "radio" || type == "checkbox") {
							...
							// 下拉值为空或文本框文本域等为空
							$(control).testRemind((tag == "empty" && text)? Tr("您尚未输入")+ text : OBJREG.prompt[tag]);
							control.focus();
						} else if (/^range|number$/i.test(type) && Number(control.value)) {
							// 整数值与数值的特殊提示
							$(control).testRemind(Tr("值无效"));
							control.focus();
							control.select();
						} else {
							// 文本框文本域格式不准确
						...
						if(key){
							...
							if(!/^radio|checkbox$/i.test(type) && text){
								...
								if(tag == "empty"){
									customTxt=Tr("您尚未输入该项内容");
								}else{
									customTxt=Tr("您尚未选择该项内容");
								}
							}
						}
						if (target.size()) {
	...
	$.fn.extend({
		...
		testRemind: function(content, options) {
			...
			this.remind = $('<div id="'+ $.testRemind.id +'"><span class="icon-info-sign" style="font-size:1.2em;vertical-align:middle;color:#ffa830"></span> '+ content +'</div>').css(params.css);
		...
		html5Validate: function(callback, options) {
			...
			var params = $.extend({}, defaults, options || {});
			//如果不是绑定form标签，而是具体的某个字段时，返回布尔值
			if ($(this).length>0 && $(this).get(0).tagName.toLowerCase() != "form"){
				var valid = $.html5Validate.isAllpass($(this), params);
				if (valid && params.validate() && $.isFunction(callback)) {
					valid = callback.call(this);
				}
				return valid;
			}

...
func (a *Index) Index() error {
	...
	a.DB.Replica().Where(`Post.id=1`).Join(`LEFT`, `webx_user`, `User.id=Post.uid`).
		Join(`LEFT`, `webx_ocontent`, `Ocontent.rc_id=Post.id AND Ocontent.rc_type=?`, `post`).Find(&ms)
	ms2 := []*Post2{}

	log.Info(`测试extends带普通字段的查询：`)
	a.DB.Replica().Where(`Post.id=1`).Join(`LEFT`, `webx_user`, `User.id=Post.uid`).
		Join(`LEFT`, `webx_ocontent`, `Ocontent.rc_id=Post.id AND Ocontent.rc_type=?`, `post`).Find(&ms2)

	log.Info(`测试忽略Post.content字段的查询：`)
	//自动加表前缀
	a.DB.Replica().Omit(`Post.content`).Where(`Post.id=1`).Join(`LEFT`, `~user`, `User.id=Post.uid`).
		Join(`LEFT`, `~ocontent`, `Ocontent.rc_id=Post.id AND Ocontent.rc_type=?`, `post`).Find(&ms2)

	log.Info(`测试通过rel标签指定关联条件的查询：`)
	//使用rel标签
	ms3 := []*PostCollection2{}
	a.DB.Omit(`User.passwd`).Where(`Post.id=2`).Find(&ms3)
	com.Dump(ms3)

	log.Info(`测试通过alias标签指定表别名的查询：`)
	msAlias := []*Post3{}
	a.DB.Replica().Where(`a.id=1`).Join(`LEFT`, `webx_user`, `a.id=b.uid`).

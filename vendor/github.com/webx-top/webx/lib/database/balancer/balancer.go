...
type Balancer struct {
	*xorm.Engine  // master
	replicas      []*xorm.Engine
	count         uint64
	mu            sync.RWMutex
	masterCanRead bool
}

// New opens a connection to each physical db.
...
func New(driverName, sources string) (*Balancer, error) {
	...
	for i, c := range conns {
		...
		db, err := xorm.NewEngine(driverName, c)
		if err != nil {
			return nil, err
		}
	...
	if len(b.replicas) == 0 {
		b.replicas = append(b.replicas, b.Engine)
...
func (b *Balancer) Master() *xorm.Engine {
	return b.Engine
}
...
func (b *Balancer) GetAllDbs() []*xorm.Engine {
	dbs := []*xorm.Engine{}
	dbs = append(dbs, b.Engine)

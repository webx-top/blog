...
type Ormer interface {
	...
	//The joinOp should be one of INNER, LEFT OUTER, CROSS etc - this will be prepended to JOIN
	//example:
	//1. Join(`LEFT`, []string{`user`,`b`}, `a.uid=b.id`)
	//2. Join(`LEFT`, []interface{}{&User{},`b`}, `a.uid=b.id`)
	//3. Join(`LEFT`, `user`, `post.uid=user.id`).Find(&userPosts)
	Join(joinOp string, tablename interface{}, condition string) *Session
